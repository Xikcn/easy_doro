...
def get_chat_history(max_items=10):
    try:
        ...
        try:
            ...
            # 提取最近的max_items条记录
            recent_records = user_assistant_records[-max_items*2:] if len(user_assistant_records) > max_items*2 else user_assistant_records
            ...
            for i, id in enumerate(all_data.get('ids', [])):
                if i < len(all_data['metadatas']) and i < len(all_data['documents']):
                    ...
                    records.append({
                        ...
                        'document_preview': all_data['documents'][i][:50] + "..." if len(all_data['documents'][i]) > 50 else all_data['documents'][i],
